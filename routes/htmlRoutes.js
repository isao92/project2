...
module.exports = function (app) {
  ...
  app.get("/results/:keyword/:state/:experience", function (req, res) {
    ...
    if (experience == "lessThanThree") {
      findExp = ["Internship", "Jr", "Volunteer", "first-level", "Associate's"];
      console.log("lessThanThree");
      hasLoaded = true;

    } else if (experience == "threeToFive") {
      findExp = ["Bachelor's", "bachelor's", "intermediate", "middle-level", "awareness", "Four years"];
      console.log("threeToFive");
      hasLoaded = true;

    } else if (experience == "moreThanFive") {
      findExp = ["Senior", "Supervisor", "Excecutive", "top-level", "higher grade level", "Master's"];
    ...
    axios.get("https://data.usajobs.gov/api/search?LocationName=" + state + "&Keyword=" + keyword, {
      ...
      function (response) {
        ...
        for (var j = 0; j < response.data.SearchResult.SearchResultItems.length; j++) {
          
          // get id of current job posting being analyzed
          console.log(response.data.SearchResult.SearchResultItems[j].MatchedObjectId);
          ...
          for (var i = 0; i < findExp.length; i++) {
            ...
            if (hasLoaded) {
              try {
                ...
                var quaSumm = response.data.SearchResult.SearchResultItems[j].MatchedObjectDescriptor.QualificationSummary;
                ...
                if (foundExpLevel) {
                  ...
                  var matchedId = response.data.SearchResult.SearchResultItems[j].MatchedObjectId;

                  console.log("This is the matched job posting: " + matchedId);

                  var theMatchedExpLvl = [];
                  
                  theMatchedExpLvl[x]= quaSumm;
                  // console log the qualification summary of the matched exp level
                  // console.log(theMatchedExpLvl[x]); 
                  x++;
                }  
